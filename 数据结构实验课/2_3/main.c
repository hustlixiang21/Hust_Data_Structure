...
#define INFINITY 65535  //用于表示两个点之间没有边
...
/*利用迪杰斯特拉算法计算v0到其他所有点的最短路径*/
void ShortestPath_Dijkstra(MetroGraph *G, int v0, int Patharc[], float ShortPathTable[]);

/*根据最短路径值表（到vi的最短路径值）和最短路径表（存储最短路径经过点的下标）打印最短路径*/
void Print_ShortestPath(MetroGraph *G, int Vend, float Sum, const int Patharc[], const float ShortPathTable[]);
...
int main()
{
    ...
    /*****第三关代码开始*****/
    int Patharc[MAXSIZE];             //用于存储最短路径下标的数组
    float ShortPathTable[MAXSIZE];  //用于存储到各点最短路径的权值和

    char start[MAXLENGTH];
    char end[MAXLENGTH];
    scanf("%s %s",start,end);

    ShortestPath_Dijkstra(G, locate(G,start),Patharc,ShortPathTable);
    printf("%.2f %s ",ShortPathTable[locate(G,end)],start);  //先打印路径最短
    Print_ShortestPath(G,locate(G,end),ShortPathTable[locate(G,end)],Patharc,ShortPathTable);
    /*****第三关代码结束*****/
...
void ShortestPath_Dijkstra(MetroGraph *G, int v0, int Patharc[], float ShortPathTable[])
{
    int v,w,k;
    float min;
    int final[MAXSIZE] = {0};  //final[w]=1表示求得顶点v0至vw的最短路径（因为存在不可达的点但本题无）
    //初始化部分
    for(v = 0; v<G->numNodes; v++)
    {
        Patharc[v] = -1;
        ShortPathTable[v] = INFINITY;
    }
    //另外需要在最短路径数组里初始化与v0邻接的点的权值
    ShortPathTable[v0] = 0;  //该点到自己的距离为0
    final[v0] = 1;  //该源点不需要被访问
    EdgeNode *ptr = G->Stations[v0].firstedge;
    while(ptr!=NULL)
    {
        int index = locate(G,ptr->name);
        ShortPathTable[index] = ptr->distance;
        ptr = ptr->next;
    }
    for(v = 1; v<G->numNodes; v++)
    {
        ...
        final[k] = 1;
        ptr = G->Stations[k].firstedge;
        while(ptr!=NULL)
        {
            int index = locate(G,ptr->name);
            if(!final[index] && (min+ptr->distance)<ShortPathTable[index])
            {
                ShortPathTable[index] = min + ptr->distance;
                Patharc[index] = k;
            }
            ptr = ptr->next;
        }
...
void Print_ShortestPath(MetroGraph *G, int Vend, float Sum, const int Patharc[], const float ShortPathTable[])
{
    if(Patharc[Vend] == -1)
    {
        printf("%.2f %s ",Sum,G->Stations[Vend].Station_Name);
        return;
    }
    int index = Patharc[Vend];
    float distance = Sum - ShortPathTable[index];
    Print_ShortestPath(G,index,ShortPathTable[index],Patharc,ShortPathTable);
    printf("%.2f %s ",distance,G->Stations[Vend].Station_Name);
}